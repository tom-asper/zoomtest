...
import 'package:zoomtest/pages/call/components/comment_list.dart';
import 'package:zoomtest/pages/call/components/video_view.dart';
import 'package:zoomtest/utils/jwt.dart';
...
class CallScreen extends StatefulHookWidget {
  const CallScreen({
    ...
    /// The user role. 1 to specify host or co-host. 0 to specify participant
    required this.role,

    ///  The amount of time in minutes after which an idle session will end.
    ///  Default value: 40
  ...
  Widget build(BuildContext context) {
    ...
    useEffect(() {
      Future<void>.microtask(() async {
        var token = generateJwt(widget.sessionName, widget.role.toString());
        try {
          ...
          JoinSessionConfig joinSession = JoinSessionConfig(
            ...
            sessionPassword: widget.sessionPassword ?? "123",
            token: token,
            userName: widget.userName,
            audioOptions: SDKaudioOptions,
            videoOptions: SDKvideoOptions,
            sessionIdleTimeoutMins: widget.sessionIdleTimeoutMins ?? 2,
          );
    ...
    useEffect(() {
      ...
      final userJoinListener = emitter.on(EventType.onUserJoin, (data) async {
        ...
        fullScreenUser.value = remoteUserList.last;
      });
      ...
      final userLeaveListener = emitter.on(EventType.onUserLeave, (data) async {
        ...
        List<ZoomVideoSdkUser> leftUserLis =
            leftUserListJson.map((userJson) => ZoomVideoSdkUser.fromJson(userJson)).toList();
      ...
      final cloudRecordingStatusListener = emitter.on(EventType.onCloudRecordingStatus, (data) async {
        data = data as Map;
      ...
      final testMicStatusListener = emitter.on(EventType.onTestMicStatusChanged, (data) async {
        data = data as Map;
        String status = data['status'];
        debugPrint('testMicStatusListener: status= $status');
      });

      final micSpeakerVolumeChangedListener = emitter.on(EventType.onMicSpeakerVolumeChanged, (data) async {
        data = data as Map;
        int type = data['micVolume'];
        debugPrint('onMicSpeakerVolumeChanged: micVolume= $type, speakerVolume');
      });

      final cameraControlRequestResultListener = emitter.on(EventType.onCameraControlRequestResult, (data) async {
        data = data as Map;
        bool approved = data['approved'];
        debugPrint('onCameraControlRequestResult: approved= $approved');
      });

      final callOutUserJoinListener = emitter.on(EventType.onCalloutJoinSuccess, (data) async {
        data = data as Map;
    ...
    // void selectVirtualBackgroundItem() async { //TODO: Virtual background
    ...
    // void onPressShare() async { //TODO: Share
    ...
    Future<void> onPressMore() async {
      ...
      // if (supportVB) { //TODO: Virtual background
      //   options.add(
      //     ListTile(
      //       title: const Text(
        ...
        options.add(ListTile(
            ...
            onTap: () async => {
                  await zoom.liveTranscriptionHelper
                      .enableReceiveSpokenLanguageContent(!isReceiveSpokenLanguageContentEnabled.value),
                  isReceiveSpokenLanguageContentEnabled.value =
                      await zoom.liveTranscriptionHelper.isReceiveSpokenLanguageContentEnabled(),
    ...
    void onLeaveSession(bool isEndSession) async {
      ...
      debugPrint('leave session success. Is end session: $isEndSession');
      if (mounted) {
        Navigator.pop(context);
      }
    }
                              ...
                              child: Column(
                                ...
                                children: [
                                  IconButton(
                                    ...
                                    icon: isMuted.value
                                        ? Icon(Icons.mic_off_outlined, size: iconSize, color: Colors.red)
                                        : Icon(Icons.mic_outlined, size: iconSize, color: Colors.green),
                                    iconSize: circleButtonSize,
                                    tooltip: isMuted.value == true ? "Unmute" : "Mute",
                                  ),
                                  // IconButton( // TODO: Screen sharing
                                  //   onPressed: onPressShare,
                                  //   icon: isSharing.value
                                  //       ? Icon(Icons.screen_share, size: iconSize)
                                  //       : Icon(Icons.stop_screen_share_outlined, size: iconSize),
                                  //   iconSize: circleButtonSize,
                                  // ),
                                  IconButton(
                                    ...
                                    icon: isVideoOn.value
                                        ? Icon(Icons.photo_camera_front, size: iconSize, color: Colors.green)
                                        : Icon(Icons.no_accounts_sharp, size: iconSize, color: Colors.red),
                                  ),
                                  IconButton(
